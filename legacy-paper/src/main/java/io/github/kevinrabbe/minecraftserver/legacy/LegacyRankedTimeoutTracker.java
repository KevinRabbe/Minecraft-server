...
/** Ephemeral monotonic timeout tracking for disposable Ranked materializations. */
final class LegacyRankedTimeoutTracker {
    private final long timeoutNanos;
    private final LongSupplier nanoTime;
    private final Map<UUID, Long> startedAt = new HashMap<UUID, Long>();

    LegacyRankedTimeoutTracker(int timeoutSeconds) {
        this(timeoutSeconds, System::nanoTime);
    }

    LegacyRankedTimeoutTracker(int timeoutSeconds, LongSupplier nanoTime) {
        if (timeoutSeconds < 1) {
