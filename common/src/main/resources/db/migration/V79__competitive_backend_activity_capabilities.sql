-- A healthy legacy backend must not receive an activity kind whose local runtime materializer is not enabled.
-- Existing/current legacy runtimes migrate fail-closed to Ranked-only; Clan War must be explicitly enabled after its
-- complete legacy materialization/acceptance boundary is proven.

ALTER TABLE competitive_runtime_principals
    ADD COLUMN supports_ranked_arena BOOLEAN NOT NULL DEFAULT TRUE,
    ADD COLUMN supports_clan_war BOOLEAN NOT NULL DEFAULT FALSE;

CREATE INDEX competitive_runtime_principals_activity_dispatch_idx
    ON competitive_runtime_principals(
        dispatch_enabled,
...
BEGIN
    ...
    WHERE p.dispatch_enabled
      ...
      AND (
          (target_activity_kind = 'RANKED_ARENA' AND p.supports_ranked_arena)
          OR (target_activity_kind = 'CLAN_WAR' AND p.supports_clan_war)
      )
      AND b.status = 'ONLINE'
