...
| Persistent integrity diagnostics | PROVEN | bounded read-only aggregate verifier across session/transfer authority, Coin/economy custody, shared commodity- and unique-delivery claim consumption, Auction create/terminal settlement operation+delivery+provenance+ledger evidence, Bank operation/state/ledger evidence plus loaded-tier checks, Bazaar creation/fill/order/cancellation evidence, Secure Trade terminal operation+frozen-escrow+delivery+provenance+ledger reconstruction, clan treasury transfer-result/version/arithmetic/current-balance/clan-ledger reconciliation, clan storage current custody plus operation/commodity-history/withdrawal-delivery/provenance/ledger reconstruction, item-upgrade definition/evidence, skill-state↔XP-evidence plus staged-cap operation reconciliation/current catalog+cap invariants, personal/commission craft source + creation-time output + fulfilled Crafting-XP evidence, expansion resolution/history/feature/world-era consequences, Artifact create/relocate/discovery evidence and current Attunement profile identity, persistent PvE including completed Map reward settlement→participant grant→exact fulfillment delivery/profile reconciliation, clans and competitive state; Paper `/integrity` loads bundled item/skill/Attunement/Bank-tier catalogs for catalog-aware checks; the recovered mid-transaction database-failure scenario is required to finish with the aggregate verifier returning zero issues |
