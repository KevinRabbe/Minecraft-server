...
| Persistent integrity diagnostics | PROVEN | bounded read-only aggregate verifier across session/transfer authority, economy/custody, item-upgrade definition/evidence, skill-state↔XP-evidence plus staged-cap operation reconciliation/current catalog+cap invariants, expansion resolution/history/feature/world-era consequences, Artifact create/relocate/discovery evidence and current Attunement profile identity, persistent PvE, clans and competitive state; Paper `/integrity` loads bundled item/skill/Attunement catalogs for catalog-aware checks; the recovered mid-transaction database-failure scenario is required to finish with the aggregate verifier returning zero issues |
...
- operator-triggered bounded `/integrity` verification including session/transfer identity+version checks, skill progression XP/cap evidence/catalog checks, expansion resolution/history/feature/world-era consequence checks, Artifact create/relocate/discovery source evidence and active Attunement profile catalog identity, plus fail-closed development-route capability isolation;
