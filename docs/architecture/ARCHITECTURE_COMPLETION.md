...
| Persistent integrity diagnostics | PROVEN | bounded read-only aggregate verifier across session/transfer authority, Coin/economy custody, Bank operation/state/ledger evidence plus loaded-tier checks, Bazaar creation/fill/order/cancellation evidence, item-upgrade definition/evidence, skill-state↔XP-evidence plus staged-cap operation reconciliation/current catalog+cap invariants, personal/commission craft source + creation-time output + fulfilled Crafting-XP evidence, expansion resolution/history/feature/world-era consequences, Artifact create/relocate/discovery evidence and current Attunement profile identity, persistent PvE, clans and competitive state; Paper `/integrity` loads bundled item/skill/Attunement/Bank-tier catalogs for catalog-aware checks; the recovered mid-transaction database-failure scenario is required to finish with the aggregate verifier returning zero issues |
...
- operator-triggered bounded `/integrity` verification including session/transfer identity+version checks, Bank balance/tier/version/interest history and loaded-tier checks, Bazaar creation/fill/remainder/cancellation evidence, skill progression XP/cap evidence/catalog checks, personal/commission crafting source/output/XP evidence, expansion resolution/history/feature/world-era consequence checks, Artifact create/relocate/discovery source evidence and active Attunement profile catalog identity, plus fail-closed development-route capability isolation;
