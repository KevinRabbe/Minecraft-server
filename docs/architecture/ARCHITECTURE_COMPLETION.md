...
| PostgreSQL migrations/operation locking | PROVEN | `DATA_MODEL.md`, `TRANSACTIONS_AND_ANTI_DUPE.md`, migration/operation-lock code, plus JDBC fault injection proving exact retry after PostgreSQL commits successfully but the caller loses the commit acknowledgement for both Coin and a multi-authority item-delivery claim |
...
- adversarial JDBC commit-acknowledgement-loss coverage proving that committed Coin and multi-authority item-delivery transactions recover through exact idempotent retry without a second mutation;
