...
- individualized gear now has definition-owned normalized roll profiles, deterministic intrinsic roll resolution, strict crafting/profile ownership, validated runtime stat snapshots, Paper join/post-delivery caching and derived lore, Auction House roll/upgrade inspection, append-only upgrade evidence plus integrity reconciliation, and a replay-/concurrency-safe carried-item upgrade authority that atomically advances serialized player-state authority version and exact item `state_version`/`upgrade_level` under the owning session; exact upgrade economics/progression/power and live combat application remain intentionally unresolved rather than guessed;
- item definitions now also support explicit skill-based use/equip requirements that remain separate from ownership/crafting/trade: missing skill rows project as level 0, relevant skill levels can be loaded in one bounded read, eligibility returns exact unmet requirements, bundled item requirements cross-validate against the skill catalog, equipment lore and Auction House inspection expose static requirements, and current bundled items remain unrestricted unless content opts in; Paper action-level enforcement is intentionally not enabled until a real gated item exists and the relevant action/equip boundaries are wired without per-hit PostgreSQL queries;
