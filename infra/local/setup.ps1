...
max-players=$($Server.ZoneHardCapacity)
spawn-protection=0
view-distance=6
...
function Build-And-DeployPlugins {
    ...
    try {
        & $gradle clean shadowJar verifyShadowRuntimeServices
...
if (Test-Path $secretPath) {
    $secret = (Get-Content -Raw $secretPath).Trim().TrimStart([char]0xFEFF)
}
else {
